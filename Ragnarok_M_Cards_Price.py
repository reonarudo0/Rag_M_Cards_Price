...
def ManageProcess(sharedList,data):
    i=0
    for card in data['Item Name']:
        process = mp.Process(target=GetPrice, args=(i, sharedList,card))
        process.start()
        i = i + 1
        if(i%4==0):
            process.join()

def GetPrice(i,sharedList,card):
...
class RagCards():
    def __init__(self,master):
        ...
        self.refreshBtn = tk.Button(self.master, text='Refresh Prices', command=self.StartProcess)
        self.refreshBtn.place(x=200,y=33)

        self.pbar = Progressbar(self.master, mode='indeterminate', len=150)
        self.pbar.place(x=300,y=35)
    ...
    def StartProcess(self):
        self.DeleteCanvas()
        searchingLabel = tk.Label(self.canvasData, text='Procurando Preços.')
        searchingLabel.place(x=0, y=5)
        sharedList[:] = []
        self.searchBtn['state'] = 'disabled'
        self.refreshBtn['state'] = 'disabled'
        self.pbar.start(20)

        process = mp.Process(target=ManageProcess, args=(sharedList, self.data))
        process.start()
        while (len(sharedList) != len(self.data['Item Name'])):
            self.master.update()
        process.terminate()
        self.RefreshPrices()

    def RefreshPrices(self):
        self.tempList = sorted(sharedList, key=itemgetter(0))
        newPrice = [x[1] for x in self.tempList]
